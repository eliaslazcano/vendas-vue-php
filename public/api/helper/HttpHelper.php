...
class HttpHelper
{
   ...
   * @param string $url Endpoint.
   * @param array|string $formData Array associativo com chave => valor do respectivo formulário. Ou string JSON.
   * @return array|false False em caso de erro. Array com colunas "code", "type", "body". Respostas em JSON já entregarão o body adaptado.
   */
  public static function curlPost($url, $formData = null) {
    $curl = curl_init($url);
    curl_setopt_array($curl, array(
    ...
    if ($formData) {
      curl_setopt($curl, CURLOPT_HTTPHEADER, is_array($formData) ? array('Content-Type: multipart/form-data') : array('Content-Type: application/json; charset=utf-8'));
